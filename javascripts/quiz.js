$(document).ready(function (){
  var activeTest = $(".active-test").text();
  var alertmessage = "Alert : " + activeTest + " is currently available"
  $(".alert-message > h5").text(alertmessage)
								...
								$.each(value, function(index, value) {
									...
									$.each(subsections, function(key, value) {
										if (key == "questions") {
											...
											value.sort(compare);
											$.each(value, function(index, value){
												var subsectionName = subsections.name;
												value.section = subsectionName.substring(3);
...
var resultView = {
		...
		render : function() {
			...
			var revertDashboard = document.createElement("BUTTON");
			revertDashboard.innerHTML = "Back to Dashboard";
			revertDashboard.setAttribute("class", "btn btn-primary btn-xs");
			this.questionPane.append(revertDashboard);
			revertDashboard.addEventListener ("click", function() {
